...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_patch_num', dest='test_patch_num', type=int, default=20, help='Number of sample patches from testing image')
    parser.add_argument('--lr', dest='lr', type=float, default=5e-7, help='Learning rate')
    parser.add_argument('--weight_decay', dest='weight_decay', type=float, default=0.0002, help='Weight decay')
    ...
    parser.add_argument('--ranking_loss_alpha', dest='ranking_loss_alpha', type=float, default=0.0, help='Weight for ranking loss (set to 0 to disable)')
    ...
    parser.add_argument('--loss_type', dest='loss_type', type=str, default='l1', 
                       choices=['l1', 'l2', 'srcc', 'rank', 'pairwise'],
                       help='Loss function type: l1 (MAE, default), l2 (MSE), srcc (Spearman), rank (Pairwise Ranking), pairwise (Fidelity)')
    ...
    parser.add_argument('--attention_fusion', dest='use_attention', action='store_true', default=True, help='Enable attention-based multi-scale fusion (requires multi-scale to be enabled)')
    parser.add_argument('--drop_path_rate', dest='drop_path_rate', type=float, default=0.3, help='Stochastic depth rate for Swin Transformer (0.3 for Base, 0.25 for Small, 0.2 for Tiny)')
    parser.add_argument('--dropout_rate', dest='dropout_rate', type=float, default=0.4, help='Dropout rate for HyperNet and TargetNet (0.4 for Base, 0.35 for Small, 0.3 for Tiny)')
    parser.add_argument('--model_size', dest='model_size', type=str, default='base', choices=['tiny', 'small', 'base'], help='Swin Transformer model size: tiny (~28M), small (~50M), base (~88M)')
    ...
    parser.add_argument('--test_random_crop', dest='test_random_crop', action='store_true', default=True, help='Use RandomCrop for testing (original paper setup, less reproducible)')
    parser.add_argument('--no_spaq', dest='test_spaq', action='store_false', default=False, help='Disable SPAQ cross-dataset testing (saves time and memory)')
    parser.add_argument('--no_color_jitter', dest='use_color_jitter', action='store_false', default=False, help='Disable ColorJitter data augmentation (3x faster training, may slightly reduce performance)')
