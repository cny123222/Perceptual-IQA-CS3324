...
\subsubsection{Why a 4-Stage Hierarchy?} 
The 4-stage hierarchical design is crucial for capturing features at different semantic levels: Stage 1 focuses on low-level textures and local patterns critical for detecting compression artifacts and noise; Stages 2-3 extract mid-level structures important for understanding blur and distortion geometry; Stage 4 captures high-level semantic content that provides context for quality assessment. Ablation studies (Section~\ref{sec:ablation}) confirm that all four stages contribute complementarily to final performance, with each stage addressing distinct aspects of image quality perception.

\subsubsection{Why a Unified $7 \times 7$ Resolution?}
The AFA module unifies all features to a $7 \times 7$ spatial resolution, which is the native resolution of Stage 4 features from Swin Transformer. This choice is motivated by three factors: (1) it avoids lossy downsampling of the most critical semantic features from Stage 4; (2) it provides sufficient spatial detail (49 locations) for localizing distortions; and (3) it strikes an optimal balance between computational cost and representational capacity. Using larger resolutions (e.g., $14 \times 14$) would quadruple the feature dimensionality without significant accuracy gains based on our preliminary experiments.

\subsubsection{Why a 512-Channel Fusion Dimension?}
After experimenting with $\{256, 512, 1024\}$, we identified 512 as the optimal channel dimension for feature fusion. This provides the best balance: 256 channels under-represent the multi-scale information, while 1024 channels lead to overfitting on KonIQ-10k without improving cross-dataset generalization. The unified 512-dimensional features from 4 stages result in a 2048-dimensional concatenated feature vector ($512 \times 4$), which is then processed by channel attention to dynamically emphasize the most relevant scales for each image.
...
\caption{Feature map visualization for a high-quality image (MOS 81.4/100). Each stage's activation heatmap reveals where the network focuses its computational resources: Stage 1 (low-level textures) shows uniform moderate activations without distortion spikes; Stage 2-3 (mid-to-high-level structures) exhibit increasingly stronger responses aligned with semantic regions; Stage 4 (deep semantic features) maintains robust activations across content-rich areas. This \textit{``top-heavy''} activation pattern, dominated by high-level semantic stages (3-4), aligns with our channel attention analysis (Section 4.6) and indicates the model relies on semantic understanding to confirm high quality—understanding \textit{what} the image depicts suffices to verify its integrity.}
...
\caption{Feature map visualization for a low-quality image (MOS 20.1/100). The activation pattern contrasts sharply with Figure~\ref{fig:feature_high}: Stage 1 (low-level) exhibits \textit{strong, localized activations} responding to compression artifacts, noise, and texture degradation—the model's early layers detect quality-impairing distortions; Stage 2 (mid-level) shows irregular, heightened responses at distortion boundaries; Stage 3-4 (high-level semantic) display \textit{weaker overall activations} as distortions obscure semantic content. This \textit{``bottom-heavy''} activation pattern, with strong responses in low-level stages (1-2), demonstrates the model's adaptive strategy: prioritizing the detection of local distortion artifacts when semantic content is obscured, mirroring the distributed attention weights observed in our channel attention analysis.}
